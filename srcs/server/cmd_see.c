/*
** cmd_see.c for zappy in /home/chanut_g/rendu/PSU_2013_zappy/srcs/server
** 
** Made by Guillaume
...
int			cmd_see_up(t_serv *s, t_client *c, int x, int y)
{
  if (y == 1)
...
int			cmd_see_right(t_serv *s, t_client *c, int x, int y)
{
...
int			cmd_see_down(t_serv *s, t_client *c, int x, int y)
{
...
int			cmd_see_left(t_serv *s, t_client *c, int x, int y)
{
  if (x == 1)
    sprintf(c->cmd, "%i %i %i %i %i %i %i\n",
...
int			cmd_see(t_serv *s, t_client *c)
{
  printf("Client %i : voir\n", c->sock);
